...
For instance, the Metis first-order prover~\cite{hurd2003d} is integrated in multiple systems in the HOL family, but the HOL4 version is very old compared to the latest version in Isabelle/HOL.
...
The usual scenario is to decide the validity of first-order logic formula (TPTP format), but there is also support for higher-order terms (THF format), and for returning proofs expressed in a standard language (TSTP format).
